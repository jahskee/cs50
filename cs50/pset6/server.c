...
int  isfile_exist(const char* filename);
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if (request(&message, &length))
            {
                ...
                //if double space after GET, return bad request
                if(strstr(line, "GET  ") != 0) {
                    error(400);
                    continue;
                }  
                
                  //if double space before HTTP, return bad request
                if(strstr(line, "  HTTP/1.1") != 0) {
                    error(400);
                    continue;
                }  
...
char* indexes(const char* path)
{
    ...
    if (isfile_exist(indexphp) == 1){
        return indexphp;
    }
    
    if (isfile_exist(indexhtml) == 1){
        return indexhtml;
    }
